  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-6">
        <div>
          <span className="text-xs font-bold uppercase tracking-wider text-blue-400">KLH ED Cell Administration</span>
          <h1 className="text-3xl font-extrabold text-white heading-font">Admin Portal</h1>
        </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <Button variant="outline" size="sm" onClick={handleLogout} className="text-xs text-red-400 border-red-500/30 hover:bg-red-950">
      ...
      <Card className="border-blue-500/30 bg-slate-900/90 shadow-xl">
        <CardHeader>
        ...
        <CardContent className="space-y-4">
          ...
          >
            ...
            <Button
              ...
              className="bg-white text-black font-bold px-6 hover:bg-gray-200"
            >
          ...
          {generatedResult && (
            <div className="bg-emerald-950/60 border border-emerald-500/40 rounded-xl p-4 space-y-4">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-emerald-300">
                <div>
                  <h4 className="font-bold text-white text-base">Certificate Generated for {generatedResult.participantName}</h4>
                  <p className="text-xs text-emerald-400">Certificate ID: <strong className="font-mono text-white">{generatedResult.certificateId}</strong></p>
                </div>
                <a
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-slate-900/60 p-5 rounded-2xl border border-slate-800 space-y-1 backdrop-blur-xl">
          <div className="flex items-center justify-between text-slate-400">
            <span className="text-xs font-bold uppercase tracking-wider">Total Participants</span>
            <Users className="w-4 h-4 text-blue-400" />
          </div>
          <p className="text-2xl font-extrabold text-white font-mono">{participants.length}</p>
              ...
              <table className="w-full text-left text-xs text-slate-300">
                ...
                <tbody className="divide-y divide-slate-800/60 bg-slate-900/40">
                  {filteredParticipants.slice(0, 100).map((p) => (
                    <tr key={p.registration_id} className="hover:bg-slate-800/40">
                      <td className="px-4 py-3 font-mono font-bold text-blue-400">{p.registration_id}</td>
                      <td className="px-4 py-3 font-bold text-white">{p.name}</td>
                      <td className="px-4 py-3 text-slate-400">{p.team_name || "SIH 2026 Team"}</td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <Button
                          ...
                          className="text-[11px] py-1 px-3 bg-white text-black font-bold hover:bg-gray-200"
                        >
              ...
              <table className="w-full text-left text-xs text-slate-300">
                ...
                <tbody className="divide-y divide-slate-800/60 bg-slate-900/40">
                  {certificates.length === 0 ? (
                    ...
                    certificates.map((c) => (
                      <tr key={c.certificate_id} className="hover:bg-slate-800/40">
                        <td className="px-4 py-3 font-mono font-bold text-indigo-400">{c.certificate_id}</td>
                        <td className="px-4 py-3 font-bold text-white">{c.participant_name}</td>
                        ...
                        <td className="px-4 py-3 text-right space-x-2">
                          <a
                            ...
                            className="text-blue-400 font-semibold hover:underline mr-2"
                          >
                            Verify Page
                          </a>
                          <Button
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3 text-xs">
                <h4 className="font-bold text-white text-sm">Official Template: SIH-participation-template.pdf</h4>
                <p className="text-slate-400">KLH University SIH 2026 Participation Certificate Artwork</p>
                <Badge variant="success">Active PDF Template</Badge>
              </div>

              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3 text-xs">
                <h4 className="font-bold text-white text-sm font-mono">Dynamic Elements Overlay</h4>
      ...
      {activeTab === "import" && (
        <Card>
          ...
          <CardContent className="space-y-6">
            <div className="border-2 border-dashed border-slate-800 rounded-2xl p-10 text-center space-y-4 bg-slate-950 hover:bg-slate-900 transition-colors">
              <Upload className="w-10 h-10 text-blue-400 mx-auto" />
              <div className="space-y-1">
                <p className="font-bold text-slate-200 text-sm">Select or drag your CSV file</p>
              ...
              <div className="inline-block">
                ...
                <Button
                  ...
                  className="cursor-pointer bg-white text-slate-950 font-bold"
                >
                  Browse & Upload CSV
                </Button>
              </div>
            ...
            {importResult && (
              <div className="p-4 rounded-xl border border-slate-800 bg-slate-950 space-y-3">
                <h4 className="font-bold text-white text-sm">CSV Import Summary</h4>
                ...
                {importResult.rejectedCsv && (
                  <div className="pt-2">
                    ...
                    >
                      <Button variant="outline" size="sm" className="text-xs text-red-400 border-red-500/30">
